...
//        int[] indices = makeDistanceIndex(engine.getRefPersons(), discretizer);
        ...
        engine.getAttributeListeners().get(Attributes.KEY.BEELINE_DISTANCE).addComponent(mediator);
...
//            h.setPredicate(distIdxPredicate);
            ...
            */
             ...
             */
            String predicateName = String.format("%s.%s", Attributes.KEY.TRAVEL_PURPOSE, borders.get(dIdx));
    ...
    private static int[] makeDistanceIndex(Collection<? extends Person> persons, final Discretizer discretizer) {
        ...
        TaskRunner.run(new EpisodeTask() {
            ...
            public void apply(Episode episode) {
                for(Segment leg : episode.getLegs()) {
                    String val = leg.getAttribute(Attributes.KEY.BEELINE_DISTANCE);
                    if(val != null) {
